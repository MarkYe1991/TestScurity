...
public class TeammateController {
	...
	public ModelAndView getTeammates() {
		...
		return new ModelAndView("teammates")
				// Look up ALL teammates and wrap each withrelated links
				.addObject("teammates",
						StreamSupport.stream(teammateRepository.findAll().spliterator(), false)
								.map(TeammateAndLink::new).toArray())
				...
				.addObject("links", Arrays
						.asList(linkTo(methodOn(TeammateController.class).getTeammates()).withRel("All Teammates")));
	}
	...
	public ModelAndView getTeammate(@PathVariable Long id) {
		...
		return new ModelAndView("teammate").addObject("teammate", teammate).addObject("links",
				Arrays.asList(linkTo(methodOn(TeammateController.class).getTeammates()).withRel("All Teammates"),
